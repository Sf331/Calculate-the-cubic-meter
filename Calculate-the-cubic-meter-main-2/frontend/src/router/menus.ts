...
export interface MenuItem {
  path: string
  title: string
  /** 手机端底部标签栏用的短名（HIG 要求标签栏必须有文字，且越短越好）。不填就用 title。 */
  short?: string
  /** 可见角色。空数组表示所有角色可见。 */
  roles: string[]
...
export const MENUS: MenuItem[] = [
  ...
  { path: '/homework', title: '电子作业', short: '作业', roles: [] },
  { path: '/wrong-book', title: '错题本', roles: ['STUDENT'] },
  { path: '/report', title: '学情报告', short: '学情', roles: ['PRINCIPAL', 'STUDENT', 'PARENT'] },
  { path: '/lesson-account', title: '课时账户', short: '课时', roles: ['STUDENT', 'PARENT'] },
